new Vue({
    ...
    async created() {
        ...
        await axios
            .get('http://localhost:8080/allOptions')
            .then(response => (this.allReqOptions = response.data));
    },
    methods: {
        ...
        post() {
            ...
            axios({
                method: 'post',
                url: 'http://localhost:8080/options/create',
                data: param
            }).then(response => (window.location = 'http://localhost:8080/options')).catch(err => {
                if (err.response) {
                    console.log(err);
